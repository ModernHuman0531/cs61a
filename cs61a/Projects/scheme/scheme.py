...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        procedure = scheme_eval(first, env)# Identify what symbol it is
        # Since rest is a pair link, if we want to apply all element with function f, we can use Pair.map(f)
        # Recall that f is 1 element function, but scheme_eval is 2 ele,ents function.
        # We can utilize lambda function
...
def eval_all(expressions, env):
    ...
    """
    # BEGIN PROBLEM 7
    # If the next is nil, then return the first evaluate
    if expressions is nil:
        return None
    elif expressions.rest is nil:
        return scheme_eval(expressions.first, env) 
    else:
        # Evaluate the exp.first, then keep going evaluate the rest part.
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        # If the name is founded in current frame, return it's value.
        # If no find in current frame, then find it's parent frame.
        if symbol in self.bindings.keys():
            return self.bindings[symbol]
        # If no found and still have parent frame, then use recursive to keep finding the symbol.
        elif self.parent is not None:
            return self.parent.lookup(symbol)
        # END PROBLEM 2
        #If we find through all frame and don't find the symbol, then raise the Error
    ...
    def make_child_frame(self, formals, vals):
        ...
        # Bind all formals to values in the new frame
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        # 1.Convert scheme list into python list
        # If we directly change the content of the args, it might cause error, so we shall make a copy of that
        while args is not nil:
            python_args.append(args.first)
            args = args.rest
        # 2.If the self.use_env is True(function need env to be passed in), then add the current environment as last parameter 
        # in python_args
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        # Make a new frame by make_child_frame, my new frame should be a child pf the frame lambda is defined.
...
def add_builtins(frame, funcs_and_names):
    """Enter bindings in FUNCS_AND_NAMES into FRAME, an environments the second oper frame,
...
def do_define_form(expressions, env):
    ...
    >>> env = create_global_frame()
    >>> do_define_form(read_line("(x 2)"), env)Pai
    'x'
    >>> scheme_eval("x", env)
    2
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        # Take the first element out as parameter, and evaluate the rest 
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        # target = (f x y) => Pair instance, expression = ((f x y) (+ x y))
        # formal = (x y) = target.rest,body = (+ x y) = expression.rest
...
def do_lambda_form(expressions, env):
    ...
    formals = expressions.first# (x y)
    validate_formals(formals)
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # First is the parameter and the rest is the body of the lambda.
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    elif expressions.rest is nil:# The last case
        return scheme_eval(expressions.first,env)
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    # If test is false, then keep doing the recursion.
    elif expressions.rest is nil:
        return scheme_eval(expressions.first, env)
    elif is_false_primitive(scheme_eval(expressions.first, env)):
        return do_or_form(expressions.rest, env)
    else:
        # If encounter the true, then just return the evaluated value
        return scheme_eval(expressions.first, env)
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            # If the expression is true, then find the coressponding value.
            if clause.rest is not nil:
                return eval_all(clause.rest, env)
            else: # Only have the condition, then just evaluate the condition's result 
                return test
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    "*** YOUR CODE HERE ***"
    # Modify the namse and values.
    while bindings is not nil:
        # names and value are both be Pair instance.
        validate_form(bindings.first, 2, 2)# Check if the form of define only have 2 parameter
        names = Pair(bindings.first.first, names)
        # ((x 5)) = Pair(Pair(x, Pair(5, nil)), nil)
