...
class Pair(object):
    ...
    def __init__(self, first, second):
        #first and second can both store anything, but when second store another list or List it's a list
        self.first = first
        self.second = second
...
def scheme_read(src):
    ...
    if val == 'nil':
        return nil
    elif val not in DELIMITERS:  # ( ) ' . ,just a base case like number or symbol.
        return val
    elif val == "(":
...
def read_tail(src):
    ...
    if src.current() == ")": # Match the val "(" up there, means no content, so we're done
        src.pop()
        return nil
