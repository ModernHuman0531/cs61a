...
class Restaurant:
    ...
    def similar(self, k, similarity = reviewed_both):#Pass in the similarity func that determine how similar two restaurant are
        ...
        diffeerent = lambda r: -similarity(r, self)
        return sorted(other, key = diffeerent)[:k]#Key function must be one argument function
    def __repr__(self):
        return '<' + self.name +'>'
...
results = search('Cafe')# Key word we search
for r in results:
